...
class SoftCategoricalProbabilityDistributionType(ProbabilityDistributionType):
    def __init__(self, ncat):
        self.ncat = ncat
        super(SoftCategoricalProbabilityDistributionType, self).__init__()
...
class SoftCategoricalProbabilityDistribution(ProbabilityDistribution):
    def __init__(self, logits):
        self.logits = logits
        super(SoftCategoricalProbabilityDistribution, self).__init__()
    ...
    def kl(self, other):
        a0 = self.logits - U._max(self.logits, axis=1, keepdims=True)
        a1 = other.logits - U._max(other.logits, axis=1, keepdims=True)
        ea0 = tf.exp(a0)
        ea1 = tf.exp(a1)
        z0 = U._sum(ea0, axis=1, keepdims=True)
        z1 = U._sum(ea1, axis=1, keepdims=True)
        p0 = ea0 / z0
        return U._sum(p0 * (a0 - tf.log(z0) - a1 + tf.log(z1)), axis=1)
    ...
    def entropy(self):
        ...
        z0 = U._sum(ea0, axis=1, keepdims=True)
        p0 = ea0 / z0
        return U._sum(p0 * (tf.log(z0) - a0), axis=1)
