...
from drl_negotiation.core.config.hyperparameters import (GOAL, DISCRETE_ACTION_SPACE, DISCRETE_ACTION_INPUT,
                                                         ONLY_SELLER, RENDER_INFO)
    ...
    def possible_agents(self):
        if not ONLY_SELLER:
            return np.reshape(np.array(
                [[f"{agent.agent_scope_name}_seller", f"{agent.agent_scope_name}_buyer"] for agent in self.agents]),
